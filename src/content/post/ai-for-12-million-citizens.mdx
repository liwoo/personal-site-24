...
We began with a plain [RAG](https://en.wikipedia.org/wiki/Retrieval-augmented_generation) stack, [LangChain](https://www.langchain.com) and [Postgres](https://www.postgresql.org). To push accuracy up, we tried [RAG Fusion](https://arxiv.org/abs/2402.03367): let the LLM rewrite a user's question into 3 variations to widen the blast radius of what we retrieved. We also stopped staring at [BLEU](https://en.wikipedia.org/wiki/BLEU) and [ROUGE](https://en.wikipedia.org/wiki/ROUGE_%28metric%29) scores (those measure translation quality, not intelligence) and put document relevance at the centre of how we judged ourselves.
